var rule = {
    ...
    searchUrl: 'https://www.huya.com/search?hsk=**',
    searchable: 2,
    quickSearch: 0,
    // 保持搜索用的headers不变
    headers: {
    ...
    console.log('房间ID:', rid);
    
    // 方法1：使用已知的代理API（从抓包数据中获取）
    let proxyApis = [
        'http://dh.baicanuc.cn:3000/getHuyaData?rid=' + rid,
        'https://api.xinxiyu.net/api/huya?rid=' + rid,
        'https://www.freeok.xyz/huya.php?id=' + rid,
        'https://www.momoyu.cc/api/huya?rid=' + rid
    ];
    ...
    // 尝试多个代理API
    for (let apiUrl of proxyApis) {
        try {
            console.log('尝试代理API:', apiUrl);
            let proxyResponse = request(apiUrl, {
                headers: {
            ...
            console.log('API响应:', proxyResponse.substring(0, 200) + '...');
            
            // 尝试多种方式解析响应
            // 1. 直接是URL
            if (proxyResponse.startsWith('http')) {
                finalUrl = proxyResponse.trim();
                console.log('直接获取到URL:', finalUrl);
                break;
            }
            
            // 2. JSON格式
            try {
                let jsonData = JSON.parse(proxyResponse);
                if (jsonData.url) {
                    finalUrl = jsonData.url;
                    console.log('JSON格式URL:', finalUrl);
                    break;
                } else if (jsonData.data && jsonData.data.url) {
                    finalUrl = jsonData.data.url;
                    console.log('JSON data格式URL:', finalUrl);
                    break;
                } else if (jsonData.playUrl) {
                    finalUrl = jsonData.playUrl;
                    console.log('JSON playUrl格式:', finalUrl);
                    break;
                }
            } catch (e) {
                // 不是JSON，继续尝试
            ...
            for (let pattern of urlPatterns) {
                let match = proxyResponse.match(pattern);
                if (match) {
                    finalUrl = match[1] || match[0];
                    console.log('正则匹配到URL:', finalUrl);
                    break;
                }
            }
            ...
            console.log('代理API请求失败:', e.message);
            continue;
        }
    }
    
    // 方法2：如果代理API都失败，使用虎牙官方API
    if (!finalUrl) {
        console.log('所有代理API失败，使用虎牙官方API...');
        
        // 尝试m.huya.com移动端页面
        let mobileUrl = 'https://m.huya.com/' + rid;
        try {
            let html = request(mobileUrl, {
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Linux; Android 12; NOH-AL10 Build/HUAWEINOH-AL10; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/114.0.5735.196 Mobile Safari/537.36',
            ...
            // 提取stream信息
            let streamMatch = html.match(/stream\\s*:\\s*({[^}]+})/);
            if (streamMatch) {
                try {
                    let streamInfo = JSON.parse(streamMatch[1].replace(/\\\\/g, ''));
                    if (streamInfo.data && streamInfo.data[0] && streamInfo.data[0].gameStreamInfoList) {
                        let streams = streamInfo.data[0].gameStreamInfoList;
                        
                        // 优先找高清源
                        for (let stream of streams) {
                            if (stream.sCdnType === 'AL' || stream.sFlvUrl.includes('al.flv.huya.com')) {
                                let antiCode = stream.sFlvAntiCode;
                                // 解码antiCode中的HTML实体
                                antiCode = antiCode.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"');
                                
                                finalUrl = stream.sFlvUrl + '/' + stream.sStreamName + '.' + stream.sFlvUrlSuffix + '?' + antiCode;
                                console.log('提取到AL线路:', finalUrl);
                                break;
                            }
                        }
                        
                        // 如果没有AL线路，用第一个
                        if (!finalUrl && streams.length > 0) {
                            let stream = streams[0];
                            let antiCode = stream.sFlvAntiCode;
                            antiCode = antiCode.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"');
                            
                            finalUrl = stream.sFlvUrl + '/' + stream.sStreamName + '.' + stream.sFlvUrlSuffix + '?' + antiCode;
                            console.log('使用第一条线路:', finalUrl);
                        }
                    }
                } catch (e) {
                    console.log('解析stream失败:', e.message);
                }
            }
        } catch (e) {
            console.log('移动端页面请求失败:', e.message);
        }
    }
    ...
    // 如果是al.flv.huya.com地址，确保参数完整
    if (finalUrl.includes('al.flv.huya.com')) {
        // 解析URL
        let urlObj = new URL(finalUrl);
        let params = new URLSearchParams(urlObj.search);
        
        // 添加或确保必要参数
        if (!params.has('txyp')) params.set('txyp', 'o:hf3;');
        if (!params.has('fs')) params.set('fs', 'bgct');
        if (!params.has('sphdcdn')) params.set('sphdcdn', '');
        if (!params.has('sphdDC')) params.set('sphdDC', '');
        if (!params.has('sphd')) params.set('sphd', '');
        if (!params.has('exsphd')) params.set('exsphd', '');
        if (!params.has('ratio')) params.set('ratio', '0');
    ...
    // 设置headers（完全模仿抓包数据中的TV播放器）
    finalHeaders = {
        'User-Agent': 'com.fongmi.android.tv/5.1.4 (Linux;Android 12) AndroidXMedia3/1.9.0',
        'Icy-MetaData': '1',
        'Range': 'bytes=0-',
    ...
    // 重要：如果是CDN地址，需要设置正确的Host
    if (finalUrl.includes('.tbcache.com')) {
        let host = new URL(finalUrl).host;
    ...
    try {
        console.log('测试播放地址...');
        let testResponse = request(finalUrl, {
            method: 'HEAD',
        ...
        console.log('测试响应状态码:', testResponse.statusCode);
        
        // 如果是302重定向，获取重定向地址
        if (testResponse.statusCode === 302 || testResponse.statusCode === 301) {
            let location = testResponse.headers['Location'] || testResponse.headers['location'];
            if (location) {
                console.log('获取到重定向地址:', location);
                finalUrl = location;
        ...
        console.log('地址测试失败，可能无法播放:', e.message);
    }
    
    console.log('最终播放配置准备完成');
...
`,
    ...
    二级: '*',
    搜索: `js:
        input = input.replace(/(http|https):\\/\\/(www\\.)?huya\\.com\\/search\\?hsk=/, '');
        let url = 'https://www.huya.com/search?hsk=' + encodeURIComponent(input);
        ...
        try {
            // 方法1: 提取相关直播
            let livePattern = /<li class="game-live-item">[\\s\\S]*?href="https:\\/\\/www\\.huya\\.com\\/(\\d+)"[^>]*>([\\s\\S]*?)<\\/a>[\\s\\S]*?<a[^>]*title="([^"]+)"[\\s\\S]*?<img[^>]*src="([^"]+)"[^>]*alt="([^"]+)"[\\s\\S]*?<i class="js-num">([^<]+)<\\/i>/g;
            let match;
            ...
            // 方法2: 提取相关主播
            if (d.length < 10) {
                let hostPattern = /<li[^>]*class="host-item"[^>]*>([\\s\\S]*?)<\\/li>/g;
                html.replace(hostPattern, function(match, content) {
                    let roomIdMatch = content.match(/href="https:\\/\\/www\\.huya\\.com\\/(\\d+)"/);
                    let imgMatch = content.match(/<img[^>]*src="([^"]+)"[^>]*alt="([^"]+)"/);
                    let nickMatch = content.match(/<div class="nick">([\\s\\S]*?)<\\/div>/);
                    let fansMatch = content.match(/<div class="room">粉丝数: ([^<]+)<\\/div>/);
                    
                    if (roomIdMatch && imgMatch) {
                        let roomId = roomIdMatch[1];
                        let img = imgMatch[1];
                        let anchor = imgMatch[2];
                        let title = nickMatch ? nickMatch[1].replace(/<[^>]+>/g, '').trim() : anchor;
                        let fans = fansMatch ? fansMatch[1] : '';
                        
                        d.push({
                            title: title,
                            img: img,
                            desc: fans ? '粉丝: ' + fans : '主播',
                            url: 'https://www.huya.com/' + roomId
                        });
                    }
            ...
            // 方法3: 提取所有房间链接
            if (d.length === 0) {
                let roomLinks = html.match(/https:\\/\\/www\\.huya\\.com\\/\\d+/g);
                if (roomLinks) {
                    roomLinks = [...new Set(roomLinks)]; // 去重
                    for (let link of roomLinks.slice(0, 20)) {
