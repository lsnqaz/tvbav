...
class Spider(Spider):
    ...
    def playerContent(self, flag, pid, vipFlags):
        ...
        if not url:
            return {"url": self.error_url, "header": self.headers, "parse": 0}
        
        # 判断是否为视频格式链接
        video_formats = ['.m3u8', '.mp4', '.flv', '.ts']
        is_video_link = any(fmt in url.lower() for fmt in video_formats)
        
        if is_video_link:
            # 视频格式链接，直接播放
            return {"url": url, "header": self.headers, "parse": 0}
        else:
            # 网页链接，需要解析
            return {"url": url, "header": self.headers, "parse": 1}
